...
import { WAGE_TYPES_SHORT, PERSONNELS, TIMES, WAGE_MIN, METHODS } from "@/constants";
import { createPost } from "@/apis";

export const ShortWriteForm = () => {
    ...
    const { mutate } = useMutation({
        ...
        onSuccess: () => {
            alert("공고가 등록되었습니다");
            router.replace("/my/ss");
        },
    });

    const onSubmit = (data) => {
        const { date, begin, end, wage_type, wage, methods } = data;
        const b = Number(begin);
        const e = Number(end);
        const hour = b < e ? e - b : 24 + (e - b);
        const wage_hour = wage_type === "H" ? wage : Math.floor(wage / hour);
        const days = String(dayjs(date).get("d") === 0 ? 6 : dayjs(date).get("d") - 1);
        const wage_day = wage_type === "D" ? wage : Math.floor(wage * hour);
        const variables = { ...data, wage, days, hour, wage_day, wage_hour, methods: methods.join(","), type: "S" };
                        ...
                        <div>
                            ...
                            <div className="flex space-x-3 mb-3">
                                <select className="select" {...register("wage_type")}>
                                    {WAGE_TYPES_SHORT.map(({ label, value }) => (
